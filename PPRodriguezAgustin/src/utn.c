...
int getStringDos(char* bufferString,int limite);
...
static int getInt(int* pBuffer){
    ...
    int retorno =-1;
    if(getStringDos(bufferString,200)==0 && isInt(bufferString)==0){
...
int utn_getLetras(char *pBuffer,int limite,int reintentos,char* msj,char*msjError){
    ...
    if(pBuffer!=NULL && limite >0 && reintentos >=0){
        do{
            reintentos--;
            printf("\n%s",msj);
            if(getStringDos(buffer,limite)==0 && isLetras(buffer)==0){
...
int utn_getCuit(char *pBuffer,int limite,int reintentos,char* msj,char*msjError){
    ...
    if(pBuffer!=NULL && limite >0 && reintentos >=0){
        do{
            reintentos--;
            printf("%s",msj);
            if(getStringDos(buffer,limite)==0 && isCuit(buffer)==0){
                strncpy(pBuffer,buffer,limite);
                retorno=0;
                break;
            }else
